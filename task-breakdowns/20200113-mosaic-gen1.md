...
1. Implement function `generateGenesis` to generate the genesis file
    ...
    - Get the bytecode of all the contract using the function `getBytecode` and write them in `alloc` as
      ```text
      ...
      ```
    - Save the generated genesis file to the IPFS and store the CID to the `genesisFiles` model
