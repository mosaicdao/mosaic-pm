...
1. Implement `Protocore::openMetablock()` external onlyCoconsensus
1. Implement `Protocore::_proposeLink()` internal
1. Implement `Protocore::_registerVote()` internal and emit event `Protocore::VoteRegistered`, `Protocore::LinkUpvoted`
1. Implement `Protocore::hashVoteMessage()` internal
1. Implement `GenesisOriginProtocore`, it has `genesis{LinkParams}`
1. Implement `OriginProtocore::setup()` external
1. Implement `OriginProtocore::proposeLink()` external and emit `OriginProtocore::LinkProposed`
1. Implement `OriginProtocore::registerVote()` external (no inclusion principle)
1. Implement `GenesisSelfProtocore`, it has `genesis{LinkParams}`
1. Implement `SelfProtocore::setup()` external
1. Implement `SelfProtocore::proposeLink()` external (has inclusion principle) and emit `SelfProtocore::LinkProposed`
